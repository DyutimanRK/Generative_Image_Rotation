...
def summarize_performance(step, g_model, dataset, direct, n_samples=3):
    ...
    filename1 = 'plot_%07d_%s.png' % ((step+1), FD)
    plt.savefig(f"{direct}/{filename1}")
    plt.close()
    # save the generator model
    filename2 = 'modelWeight_%07d_%s.h5' % ((step+1), FD)
...
#         print("G weights:", g_model.get_weights())
        ...
        if (i+1) % (bat_per_epo * 5) == 0: # saving model and plot at every 5th epoch
            ...
            if(boolean):
                ...
                formatted_date = (dt.now()).strftime("%Y-%m-%d %H:%M:%S")
                FD = "ModelWeight"+formatted_date[2:10]+"_"+formatted_date[11:13]+"_"+formatted_date[14:16]
