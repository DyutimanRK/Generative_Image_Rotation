...
# Function to get the latest folder based on timestamp that starts with "ATTN"
def get_latest_folder(directory):
    """Returns the folder starting with 'ATTN' with the most recent timestamp."""
    try:
        # Filter folders that start with "ATTN"
        folders = [
            os.path.join(directory, f) for f in os.listdir(directory)
            if os.path.isdir(os.path.join(directory, f)) and f.startswith("ATTN")
        ]
...
# Set up CSV logging
if do_you_wish_to_resume_training:
    csv_file = os.path.join(readings_dir, f"{pxl}_ATTN_Training_Log.csv")
else:
    formatted_date = (dt.now()).strftime("%Y-%m-%d_%H-%M-%S")
    csv_file = os.path.join(readings_dir, f"{pxl}_ATTN_Training_Log_{formatted_date}.csv")
...
def summarize_performance(epch, g_model, dataset, direct, n_samples=3):
    ...
    # Save plot to file
    filename1 = f'ATTN_plot_EP{epch:03d}_{FD}.png'
        ...
        if (i + 1) % (bat_per_epo * save_interval) == 0:  # saving model and plot at every save_interval epoch
            ...
            if (boolean):
                if last_model:  # If the model is loaded
                    ...
                    formatted_date = (dt.now()).strftime("%Y-%m-%d %H-%M-%S")
                    new_folder_name = f"ATTN_ModelWeight_{formatted_date[2:10]}_{formatted_date[11:13]}_{formatted_date[14:16]}"
                    ...
                    formatted_date = (dt.now()).strftime("%Y-%m-%d %H-%M-%S")
                    FD = f"ATTN_ModelWeight_{formatted_date[2:10]}_{formatted_date[11:13]}_{formatted_date[14:16]}"
